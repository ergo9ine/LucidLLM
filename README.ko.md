...
> **주요 특징:** 18,200+ 라인 코드 • 4 개 언어 지원 • WebGPU/WASM 추론 • 토큰 스트리밍 • 커스텀 가상 DOM • AES-256 암호화 백업 • OPFS 모델 캐싱
...
| **폴백 지원** | WASM 지원 브라우저 |
| **보안 컨텍스트** | HTTPS 또는 localhost 필요 (OPFS) |
...
- **네이티브 아키텍처** — 고성능 동적 UI 렌더링을 위해 커스텀 가상 DOM (vdom.js) 을 탑재한 Vanilla JS 코어.
...
### 1. 고성능 가상 DOM (Virtual DOM)
복잡한 UI 업데이트 (모델 테이블, 채팅 목록 등) 를 효율적으로 처리하기 위해 경량 커스텀 가상 DOM (`vdom.js`) 을 사용합니다. 이는 외부 의존성 없이 고성능 렌더링을 가능하게 합니다.
