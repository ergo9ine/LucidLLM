...
| **토큰 속도 통계** | 초당 토큰 수(TPS) 통계(평균/최대/최소)를 실시간으로 표시합니다. |
| **메모리 사용량 표시** | 실시간 메모리 소비량을 모니터링합니다. |
...
| **자동 스크롤** | 생성 중 자동으로 하단으로 스크롤되며, 수동 오버라이드 버튼을 지원합니다. |
...
| **Gzip 압축** | 백업 페이로드를 선택적으로 Gzip 압축합니다. |
| **자동 백업** | 데이터 변경 시 자동으로 백업을 수행합니다(디바운싱 25초 적용). |
| **백업 복원** | 특정 시점의 백업 스냅샷으로 복원할 수 있습니다. |
...
| **4가지 테마** | 다크 모드, 라이트 모드, OLED 블랙 모드(순수 블랙), 하이 콘트라스트 모드를 지원합니다. |
| **반응형 디자인** | 모바일, 태블릿, 데스크탑 등 모든 기기에 최적화된 UI를 제공합니다. |
| **PWA 지원** | 프로그레시브 웹 앱 기능 (예정) |
| **사이드바 네비게이션** | 채팅 및 워크스페이스 패널이 있는 접을 수 있는 사이드바를 제공합니다. |
| **단축키 지원** | Ctrl+N (새 채팅), Ctrl+Enter (전송), Ctrl+L (입력창 포커스), Ctrl+, (설정), Ctrl+Shift+Backspace (삭제), Ctrl+Shift+E (내보내기), Ctrl+B (사이드바), Ctrl+/ (도움말) |
...
| Qwen2.5-0.5B-Instruct | ~500M | Q4 | 균형 잡힌 성능 |
| Phi-4-mini-instruct | ~3.8B | Q4 | 고품질 응답 |
...
1. 하단 입력창에 메시지를 입력하고 **전송** 버튼을 누르거나 `Ctrl+Enter`를 누릅니다.
2. 탭 바의 **+** 버튼으로 새로운 채팅 세션을 만들 수 있습니다.
3. 탭을 클릭하여 여러 대화 세션을 자유롭게 전환할 수 있습니다.
4. **중단** 버튼(또는 `Ctrl+Shift+Backspace`)으로 언제든지 생성을 중단할 수 있습니다.
...
3. **자동 백업**을 활성화하면 변경 사항이 생길 때마다 자동으로 저장됩니다 (25초 디바운싱).
...
- 주요 파일:
  - `script/bootstrap.js` — 초기화 및 하이드레이션
  - `script/main.js` — UI 상태/핸들러 및 렌더링
  - `script/i18n.js` — 다국어 처리 모듈 (한국어, 영어, 일본어, 중국어 간체)
  - `script/worker.js` — 추론 워커 및 파이프라인 관리
  - `script/shared-utils.js` — 공용 유틸리티
  - `script/drive-backup.js` — 암호화 백업 로직
- 테스트: Vitest 단위 테스트 (`npm test`), Playwright E2E 테스트 (`npx playwright test`)
- 디버깅: DevTools에서 `state` 콘솔 확인, `opfs` 매니페스트 및 `transformers` 파이프라인 캐시 검토
