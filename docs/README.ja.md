...
| **パフォーマンス監視** | 1秒あたりのトークン数（TPS）統計（平均/最大/最小）とメモリ使用量をリアルタイムで表示します。 |
...
| **自動バックアップ** | データ変更時に自動的にバックアップを実行します（デバウンス適用）。 |
...
| **テーマオプション** | ダークモード、ライトモード、OLEDブラックモードをサポートしています。 |
| **レスポンシブデザイン** | モバイル、タブレット、デスクトップなど、あらゆるデバイスに最適化されたUIを提供します。 |
| **ショートカットキー** | 新規チャット（Ctrl+Shift+N）、エクスポート（Ctrl+Shift+E）、サイドバートグル（Ctrl+B）など、生産性を高めるショートカットを提供します。 |
...
1. 下部の入力欄にメッセージを入力し、**送信** ボタンを押すか `Enter` キーを押します。
...
3. **自動バックアップ** を有効にすると、変更があるたびに自動的に保存されます。
